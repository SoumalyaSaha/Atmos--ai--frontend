...
// EXTENDED FAQ CACHE - covers 90% of common questions without API call
const FAQ_CACHE = {
  // Carbon footprint basics
  'what is carbon footprint': 'A carbon footprint is the total greenhouse gases (CO₂ + methane) generated by your actions. The average Indian produces ~2 tons/month.',
  'how to reduce carbon footprint': 'Top 5 ways: 1) Use public transport, 2) Eat less meat, 3) Switch to LED, 4) Buy local, 5) Reduce waste.',
  'why track carbon footprint': 'Tracking helps you identify high-emission habits and measure your progress. Small changes add up to big impact!',
  'what is my carbon footprint': 'Your carbon footprint is calculated from transport, home energy, diet, shopping, and waste habits. Complete the calculator to see yours!',

  // App specific
  'what is atmos': 'Atmos is your AI-powered carbon footprint tracker. It calculates emissions using India-specific factors, gives personalized insights, and gamifies your eco-journey.',
  'how does atmos work': 'Atmos tracks your daily activities → calculates emissions using India-specific factors → gives AI insights → gamifies with challenges and badges.',
  'how to use atmos': '1) Sign in with Google, 2) Enter your age, 3) Fill the calculator, 4) View dashboard, 5) Complete challenges, 6) Chat with AI for tips!',

  // Gamification
  'how to earn eco points': 'Earn points by: daily login (+2), completing challenges (+100), maintaining streaks (+5/day), and reducing footprint (+10 per kg saved).',
  'what are eco points': 'Eco points are your reward for sustainable actions. Earn them to level up and unlock badges!',
  'how to level up': 'Level up by earning eco points! Level 2 needs 100 pts, Level 3 needs 300, Level 4 needs 600, and so on.',
  'what are challenges': 'Challenges are sustainability goals like "Zero Plastic Week" or "Green Commuter." Complete them for eco points, CO₂ savings, and badges.',
  'how to complete challenges': 'Start a challenge from the Challenges page, then check in daily. Complete all days to finish and earn rewards!',
  'what are badges': 'Badges are achievements you unlock by hitting milestones: First Step, Savings Star, Eco Warrior, Streak Master, etc.',
  ...
  'how to reduce home energy': 'Switch to LED, use star-rated appliances, insulate your home, and consider rooftop solar for long-term savings.',
  'best cooking fuel': 'Biogas = cleanest (0.05 kg). LPG = 2.98 kg. Natural gas = 2.1 kg. Electricity = 0.71 kg/unit.',

  // Diet
  'how to reduce diet emissions': 'Meatless Mondays save ~8 kg/month. Going vegetarian saves ~40 kg/month. Vegan saves ~60 kg/month.',
  'best diet for environment': 'Vegan = 1.5 kg/day (lowest). Vegetarian = 2.0 kg. Meat occasional = 2.8 kg. Meat daily = 4.5 kg (highest).',
  'is meat bad for environment': 'Meat production uses 10x more land and water than plants. Reducing meat is the #1 way to cut diet emissions.',

  // Shopping
  'how to shop sustainably': '1) Buy second-hand (save 50%), 2) Repair vs replace, 3) Quality over quantity, 4) "30 wears" rule for clothes.',
  'reduce shopping emissions': 'Electronics = 0.05 kg/₹. Clothing = 0.015 kg/₹. Buy less, buy used, buy local!',

  // Waste
  'how to reduce waste': 'Compost food scraps, recycle paper/plastic/glass, avoid single-use plastics, buy in bulk to reduce packaging.',
  'best waste disposal': 'Composted = 0.02 kg/bag (best). Recycled = 0.05 kg. Landfill = 0.5 kg (worst).',

  // India specific
  'india average carbon footprint': 'India average is ~1.9 tons CO₂/person/year (global avg is 4.8 tons). Urban Indians average ~2.5 tons.',
  'is my footprint good': 'Compare with India average for your age group. Under average = great! Above = room for improvement.',

  // General tips
  'quick eco tips': '1) Carry a water bottle, 2) Use cloth bags, 3) Take shorter showers, 4) Eat local seasonal food, 5) Work from home when possible.',
  'easy ways to go green': 'Start small: reusable bags, LED bulbs, shorter showers, meatless Mondays, public transport. Every bit counts!',

  // Help
  'help': 'I can answer questions about: your carbon footprint, how to reduce emissions, challenges, eco points, diet/transport/energy tips, and using Atmos. What would you like to know?',
  'what can you do': 'I analyze your carbon footprint data and give personalized advice. Ask me about transport, diet, energy, shopping, waste, challenges, or general eco tips!',
}
...
function findFAQAnswer(input) {
  ...
  // Direct match
  if (FAQ_CACHE[lower]) return FAQ_CACHE[lower]

  // Keyword matching
  for (const [question, answer] of Object.entries(FAQ_CACHE)) {
    const qWords = question.split(' ')
    const inputWords = lower.split(' ')

    // Check if most keywords match
    const matches = qWords.filter(w => inputWords.includes(w)).length
    if (matches >= Math.min(2, qWords.length * 0.5)) {
      return answer
    }
...
export default function Chatbot() {
  ...
  const buildWelcomeMessage = () => {
    if (!user?.onboardingComplete) {
      return `Hi! I'm Atmos Assistant. To give you personalized advice, please complete the calculator first. I can then analyze your emissions and suggest ways to reduce them!`
    }
  ...
  const sendMessage = async () => {
    ...
    try {
      // STEP 1: Check FAQ cache (ZERO API CALL)
      const faqAnswer = findFAQAnswer(userMessage.content)

      if (faqAnswer) {
        console.log('[CHAT] FAQ cache hit - 0 API calls')
      ...
      // STEP 2: Build context-aware prompt for AI
      console.log('[CHAT] FAQ miss - calling AI API')
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {msg.source && (
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                      msg.source === 'faq' ? 'bg-emerald-900/50 text-emerald-400' :
                      msg.source === 'ai' ? 'bg-blue-900/50 text-blue-400' :
                      'bg-gray-800 text-gray-500'
                    }`}>
                      {msg.source === 'faq' ? '⚡ Instant' : msg.source === 'ai' ? '🤖 AI' : '💾 Cached'}
                    </span>
                  )}
                </div>
              </div>
